...
function extractRequestInfo(action) {
    var originalClass = janet_ts_1.getClassByActionType(action._name);
    if (originalClass) {
        return Reflect.getMetadata(exports.metaKey, originalClass);
    }
    else {
        return null;
    }
...
function isHTTPAction(action) {
    var requestInfo = extractRequestInfo(action);
...
function createHTTPRequestFromScheme(action, scheme) {
    ...
    var body = scheme.body != null ? scheme.body.propertyAccessor.readValue(action) : null;
...
function createHTTPRequestFromAction(action) {
    var scheme = parseHTTPAction(action);
